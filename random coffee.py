...
class pool:
    ...
    def get_participant_info(self,filename='match_history.csv'):
        with open(filename,encoding='utf-8') as file:
            reader=csv.reader(file)
            for row in reader:
                self.participants[int(row[2])]=participant(int(row[2]),row[1],row[0],[int(x) for x in row[3:] if x!=''])
                if int(row[2]) in self.not_matching:
                    self.participants[int(row[2])].matching=False


    def run_matching(self,email_text,current_match_filename='current matches.csv',formatted_matches_filename='formatted match.csv'):
                ...
                for part in participant_list:
                    if self.participants[part].new_match==None:
                        ...
                        if part not in matches_made:
                            ...
                            current_match_writer.writerow([p1.id,p2.id,p1.email+','+p2.email,'Random Coffee Pairing','Hi '+p1.first_name+' and '+p2.first_name+',\n\n'+email_text])
